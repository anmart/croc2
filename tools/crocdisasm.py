# -*- coding: utf-8 -*-
"""
GBC disassembler - Modified from the TCG version available at https://github.com/pret/poketcg
"""
...
def created_but_unused_labels_exist(byte_labels):
	...
	"""
	return (False in [label["definition"] for label in list(byte_labels.values())])

def all_byte_labels_are_defined(byte_labels):
	"""
	Check whether all labels have already been defined.
	"""
	return (False not in [label["definition"] for label in list(byte_labels.values())])
...
def create_address_comment(offset):
	comment_bank = int(offset / 0x4000)
	if comment_bank != 0:
...
class Disassembler(object):
	...
	def output_bank_opcodes(self, start_offset, stop_offset, hard_stop=False, parse_data=False, include_last_address=True):
		...
		bank_id = int(start_offset / 0x4000)
		...
		while True:
			...
			data_label_created = local_offset in list(data_tables.keys())
			byte_label_created = local_offset in list(byte_labels.keys())
			...
			if not is_data or not parse_data:
				...
				# opcodes with 1 argument
					if opcode_byte != 0xcb: # bit opcodes are handled separately
						...
						# if the current opcode is a relative jump, generate a label for the address we're jumping to
							...
							if local_target_address in list(byte_labels.keys()):
							...
							if local_target_address in list(data_tables.keys()):
					...
					# anything that isn't a call or jump is a load-based command
						...
						# handle the case of a label for the current address not existing
							...
							if offset_is_used(byte_labels, local_target_offset):
								...
								if local_target_offset in list(data_tables.keys()):
							...
							elif local_target_offset in list(data_tables.keys()):
				...
				if get_local_address(offset) in list(byte_labels.keys()):
					is_data = False
			...
			# check if this is the end of the function, or we're processing data
				...
				if local_offset not in list(byte_labels.keys()) and local_offset in list(data_tables.keys()) and created_but_unused_labels_exist(data_tables) and parse_data:
					is_data = True
		...
		for label_line in list(byte_labels.values()):
		...
		for label_addr in list(data_tables.keys()):
			# get the label dict
...
# disasm now has to be passed as arg because I use this function in other scripts
def get_raw_addr(addr,dis=None):
	if addr:
		if ":" in addr:
			addr = addr.split(":")
			...
			if dis is not None:
				label_addr = dis.find_address_from_label(addr)
				if label_addr:
					return label_addr
			addr = int(addr, 16)
...
if __name__ == "__main__":
	...
	start_addr = get_raw_addr(args.offset,dis=disasm)
	stop_addr = get_raw_addr(args.end, dis=disasm)
