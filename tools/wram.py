...
def make_wram_labels(wram_sections):
    ...
    for section in wram_sections:
        for label in section['labels']:
            if label['address'] not in list(wram_labels.keys()):
...
class BSSReader:
    ...
    def read_bss_line(self, l):
        ...
        elif token in list(self.macros.keys()):
            ...
            if not line:
                ...
                for x in ['EQU', '=', 'SET']:
                    if x in split_line_upper:
                        ...
                        name, value = list(map(' '.join, [split_line[:index], split_line[index+1:]]))
...
def scrape_constants(text):
    ...
    return {v: k for k, v in list(constants.items())}
...
class WRAMProcessor(object):
    ...
    def reformat_wram_labels(self):
        ...
        for (address, labels) in self.wram_labels.items():
            for label in labels:
